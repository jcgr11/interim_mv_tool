...
class StreamlitApp:
    ...
    def setup_sidebar(self):
        ...
        self.beginning_value = st.sidebar.number_input(
    ...
    def collect_trades(self):
        ...
        for i in range(4):
            trade_date = st.sidebar.date_input(
                f"Trade {i+1} Date",
                self.start_date + datetime.timedelta(days=30 * (i + 1)),
            )
            shares = st.sidebar.number_input(
                f"Number of Shares for Trade {i+1}", min_value=0
            )
    ...
    def display_results(self):
        ...
        if st.button("Get Data") and self.ticker:
            st.title(
                "A simple web app for calculating returns and interim market values"
            )
            self.finance_data = FinanceData(self.ticker, self.start_date, self.end_date)
            interim_values, trade_dates = self.finance_data.calculate_interim_values(
                self.start_date, self.end_date, self.beginning_value, self.trades
            )

            st.markdown(
                f"**{self.ticker} return from {self.start_date} to {self.end_date}: {self.finance_data.geometric_mean:.4%}**"
            )
            ...
            # Plotting
            fig = self.plot_data(
                self.finance_data.price, self.finance_data.return_plot_data
            )
    ...
    def plot_data(self, price, return_plot_data):
        ...
        fig.update_layout(
            title=f"Daily {self.ticker} Price and Returns: {self.start_date} to {self.end_date}"
        )
